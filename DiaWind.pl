...
sub DiaWind { 
	...
	if($popupid[0]){ $PopId = $popupid[0]; }else{ $PopId = 0; } my $string = "popupid:" . $popupid[0]; $Text=~ s/$string//g; 
	my @wintype = $Text =~ /wintype\:(\d+)/g;
	if($wintype[0] == 1){ $Opt = 1; } else{ $Opt = 0; } my $string = "wintype:" . $wintype[0]; $Text=~ s/$string//g; 
	if(!$nobracket){
	...
	else{ if($Speaking ne ""){$d_b = '<br><br>'; } quest::popup($Title,  "<c \"#CCFF33\">" . $Speaking . "</c>" . $d_b . $quo . "" . $Text . "" . $quo . "" . "<br><br>" . $ClickQueue . "", $PopId, $Opt, $Timer); }
