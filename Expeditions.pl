...
sub GetTimeLeftAdv {
	...
	if (!$ReturnType) { $TimeLeft = $Days . $HoursLeft . "h " . $MinutesLeft . "m " . int($SecondsLeft) . "s "; }
	else {
...
sub InitInstanceQueue{
	...
	if($PlayerCount < $_[13]){ $client->Message(15, "You do not meet the requirements for this expedition: ". $_[1] . " of " . $_[13] . " players, you have " . $PlayerCount . " player(s) in your party"); }
		...
		if($Init == 0){ $client->Message(15, "This entity is offering a expedition.");
			...
			for($i=0;$i<=20;$i++){ $client->SetEntityVariable("InstRequest". $i, $_[$i]); if($Debug){ $client->Message(15, "Var " . $i . " " . $_[$i]); } }
		}
	}
}
...
sub DisplayInstanceQueue{
	if(!$_[0]){ $_[0] = GetExpeditionInstanceID(); }
...
sub LoadExpeditionInfo2{
	...
	my $query = "SELECT
		cust_inst_players.inst_id,                # 0
		cust_inst_players.player_name,            # 1
		cust_inst_players.is_leader,              # 2
		cust_ext_instances.task_assoc,            # 3
		cust_ext_instances.inst_name,             # 4
		cust_ext_instances.avglevelreq,           # 5
		cust_ext_instances.ID,                    # 6
		cust_ext_instances.shared_task,           # 7
		cust_ext_instances.duration,              # 8
		cust_ext_instances.expdate,               # 9
		cust_ext_instances.limit,                 # 10
		cust_ext_instances.lockout,               # 11
		cust_ext_instances.return_zone,           # 12
		cust_ext_instances.return_version,        # 13
		cust_ext_instances.return_instanceid,     # 14
		cust_ext_instances.return_x,              # 15
		cust_ext_instances.return_y,              # 16
		cust_ext_instances.return_z,              # 17
		cust_ext_instances.zonesn,                # 18
		cust_ext_instances.boot_on_completion     # 19
		FROM                                      # 
		cust_inst_players                         # 
...
sub GetExpeditionDescription{ @AdvInfo = LoadExpeditionInfo2(); return $AdvInfo[4]; } 
sub GetExpeditionTimeLeft{ @AdvInfo = LoadExpeditionInfo2(); return (($AdvInfo[9]) - int(time())); }
sub GetExpeditionID{ @AdvInfo = LoadExpeditionInfo2(); return $AdvInfo[6]; }
sub GetExpeditionInstanceID{ @AdvInfo = LoadExpeditionInfo2(); return $AdvInfo[0]; }
sub GetExpeditionTaskID{ @AdvInfo = LoadExpeditionInfo2(); return $AdvInfo[3]; }
sub GetIsExpeditionLeader{ @AdvInfo = LoadExpeditionInfo2(); return $AdvInfo[2]; }

sub ExpeditionMessage{
	$connect = plugin::LoadMysql();
	my $query = "SELECT `char_id`, `player_name` FROM `cust_inst_players` WHERE `inst_id` = " . GetExpeditionInstanceID() . ";";
	$query_handle = $connect->prepare($query); $query_handle->execute(); 
	while (@row = $query_handle->fetchrow_array()){ quest::crosszonemessageplayerbyname(15, $row[1], $_[0]); } 
}
...
sub ShowExpeditionCheckpointLinks{ 
	$connect = plugin::LoadMysql(); my $client = plugin::val('$client'); 
	my $query = "SELECT expedition_id, checkpoint_description, zone_id, instance_id, x, y, z, h, checkpoint_time
		FROM `cust_ext_exp_checkpoints` WHERE `expedition_id` = " . GetExpeditionID() . "";
	$query_handle = $connect->prepare($query); $query_handle->execute(); my $res = 0; 
	while (@row = $query_handle->fetchrow_array()){
		if($res != 1){ 
			$res = 1; 
			$client->Message(15, " :: Checkpoints :: ");
			$client->Message(15, "----------------------------------------------");
		}
		$client->Message(0, " :: " . quest::saylink("#doexpdcheckpoint $row[0] $row[2] $row[3] $row[4] $row[5] $row[6] $row[7]", 1, $row[1]));
	}
	if($res != 1){ $client->Message(15, "You have no current checkpoints..."); }
}

#::: Force clients to entry an expedition that are currently in the zone
sub ForceClientExpEntry{
	my $entity_list = plugin::val('$entity_list');
	$connect = plugin::LoadMysql();
	my $InstID = GetExpeditionInstanceID();
	$query = "SELECT
		instance_list.id,
		instance_list.zone,
	...
	$query_handle = $connect->prepare($query); $query_handle->execute();
	while (@row = $query_handle->fetchrow_array()){
		$entity_list->GetClientByCharID($row[1])->MovePCInstance($zone, $InstID, $X, $Y, $Z, $H); 
	}
}

sub ExpeditionListLockouts{
	my $client = plugin::val('$client');
	$connect = plugin::LoadMysql(); my $res = 0;
	$query_handle = $connect->prepare("SELECT `lockout_name`, `lockout_expire` FROM `cust_ext_lockouts` WHERE `player` = '" . $client->GetCleanName() . "'"); 
	$query_handle->execute();
	while (@row = $query_handle->fetchrow_array()){
		if($res != 1){ $res = 1; $client->Message(0, "Listing lockouts..."); } 
		$client->Message(0, "");
		$client->Message(15, "'". $row[0] . "' that expires in ". GetTimeLeftAdv($row[1]));
	}
	if($res != 1){ $client->Message(0, "You currently have no active lockouts..."); }
}

sub ExpeditionCheckpoint{
	my $client = plugin::val('$client');
	$connect = plugin::LoadMysql();
	$query_handle = $connect->prepare("REPLACE INTO `cust_ext_exp_checkpoints` (expedition_id, checkpoint_description, zone_id, instance_id, x, y, z, h, checkpoint_time) 
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)")->execute(GetExpeditionID(), $_[0], $_[1], $_[2], $_[3], $_[4], $_[5], $_[6], time()); 
	plugin::MM("[Checkpoint] '" . $_[0] . "'"); 
	ExpeditionMessage("[Checkpoint] '" . $_[0] . "'");
	$client->PlayMP3("mail2.wav");
	if($_[7] == 1){ #::: Move all to Checkpoint 
		$query = "SELECT
		inst_id,
		char_id,
		player_name
		FROM
		cust_inst_players
		WHERE inst_id = " . GetExpeditionInstanceID() . "";
		$query_handle = $connect->prepare($query); $query_handle->execute();
		while (@row = $query_handle->fetchrow_array()){
			quest::crosszonesignalclientbyname($row[2], 609);
		}
	}
}
