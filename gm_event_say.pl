sub GM_EVENT_SAY{
	...
	if($text=~/#buffs/i){
		$client->SpellFinished(10955, $client); ###  Hunter's Breastplate
		$client->SpellFinished(10956, $client); ###  Hunter's Shield
		$client->SpellFinished(278, $client); ## Spirit of Wolf
		@buffs = (2517, 5522, 14302, 18784, 19444, 19477, 18620, 19094, 6824);
		foreach $val (@buffs){ quest::selfcast($val); }
	}
	if($text=~/#armor/i){
		if($arg[1] eq "load"){
			...
			%Slots = (
				...
				7 => ["Arms", 128, 7],
				8 => ["Back", 256, 8],
				9 => ["Right Wrist", 1536, 9],
				10 => ["Left Wrist", 1536, 10],
				11 => ["Range", 2048, 11],
				12 => ["Hands", 4096, 12],
				13 => ["Primary", 8192, 13],
				14 => ["Secondary", 16384, 14],
				15 => ["Left Finger", 98304, 15],
				16 => ["Right Finger", 98304, 16],
				17 => ["Chest", 131072, 17],
				18 => ["Leggings", 262144, 18],
				19 => ["Feet", 524288, 19],
				20 => ["Waist", 1048576, 20],
			);
			%Classes = (
			...
			$n = 0;
			while($Slots{$n}[0]){
				if($n == 13){ $query = "SELECT `id`, `hp`, `classes` FROM items WHERE `slots` = '". ($Slots{$n}[1] + $Slots{$n + 1}[1]) . "' AND reclevel < " . ($ulevel + 5) . " AND reclevel > " . ($ulevel - 5) . " ORDER BY hp DESC"; }
				else {
					$query = "SELECT `id`, `hp`, `classes` FROM items WHERE `slots` = '". $Slots{$n}[1] . "' AND reclevel < " . ($ulevel + 5) . " AND reclevel > " . ($ulevel - 5) . " ORDER BY hp DESC";
				}
				...
				while (@row = $query_handle->fetchrow_array()){
					...
					$FoundSlot = 0;
					while($Classes{$c}[0]){
						if($Classv > $Classes{$c}[0]){ $Classv -= $Classes{$c}[0]; }
						elsif($Classv == $Classes{$client->GetClass()}[0]){
							$client->Message(15, "You should be able to wear this " . quest::varlink($ItemID));
							$FoundSlot = 1;
							last; 
						}
						$c--;
					...
					if($FoundSlot == 1){ 
						if($arg[2]){
							$client->SummonItem($ItemID, $Charges, $Attuned, $arg[2], $arg[2], $arg[2], $arg[2], $arg[2], $Slots{$n}[2]);
						}else{
							$client->SummonItem($ItemID, $Charges, $Attuned, $AugSlot1, $AugSlot2, $AugSlot3, $AugSlot4, $AugSlot5, $Slots{$n}[2]);
						}
						$client->SendWearChange($Slots{$n}[2]);
						#::: We Skip these slots so they can process left or right (wrist or ring etc.)
						if($Slots{$n}[2] != 15 && $Slots{$n}[2] != 9){
							last; 
						}else{ $n++; }
					}
				}
	...
	if($text=~/#taskselector/i){ quest::taskselector($arg[1]); }
	if($text=~/#taskassign/i){ quest::assigntask($arg[1]); }
	if($text=~/#dev/i){
		if(!$arg[1]){ 
			...
			$client->Message(15, "#dev killallnpc <npcname>");
			$client->Message(15, "#dev killallnpcs");
			$client->Message(15, "#dev pullallnpcs");
		}
		...
		if($arg[1] eq "pullallnpcs"){
			my @nlist = $entity_list->GetNPCList();
			foreach my $n (@nlist){
				$n->GMMove($client->GetX(), $client->GetY(), $client->GetZ(), 0);
			}
		}
		
		if($arg[1] eq "npcedit"){
			my $Target = $client->GetTarget(); my $TargetNPCID = $Target->GetNPCTypeID(); my $TargName = $Target->GetCleanName();
			plugin::DiaWind("<a href=\"http://eoc.akkadius.com/AllaClone/eoc/min.php?Mod=NPC&SingleNPCEdit=" . $TargetNPCID . "\">Edit - " . $TargName. "</a> mysterious");
		}
	...
	if($text=~/#gmzone/i && $arg[1] ne "" && $arg[2] ne ""){
		$connect = plugin::LoadMysql();
		# PREPARE THE QUERY
		$query = "SELECT
			zone.safe_x,
			...
			FROM
			zone
			WHERE short_name = '". $arg[1] . "';";
		$query_handle = $connect->prepare($query);
		$query_handle->execute();
		while (@row = $query_handle->fetchrow_array()){
			$client->Message(315, "Sending you to public instance Version - $text and $ZoneShortName, Safe Zone points - X - $row[0], Y - $row[1], Z - $row[2],");
			if($arg[3]){ $ident = $arg[3]; } else{ $ident = "GMZONE"; }
			plugin::SendToInstance("public", $arg[1], $arg[2], $row[0], $row[1], $row[2], $ident, 604800);
		}
	}
}